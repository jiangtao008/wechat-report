#!/usr/bin/env bash
# generate-index.sh — 扫描群目录下的所有 .html 文件，生成 index.json
...
echo "🔍 扫描 $ROOT 下的 HTML 报告..."

# Build JSON
json='{\n  "updated": "'$(date -u +"%Y-%m-%dT%H:%M:%SZ")'",\n  "groups": ['

first_group=true
# 扫描 report/ 目录下的所有群子目录
for group_dir in $(find ./report -maxdepth 1 -type d ! -name 'report' | sort); do
  group_name="${group_dir#./report/}"
  ...
  for f in "${html_files[@]}"; do
    ...
    relpath="report/${group_name}/${basename_f}"
